---
title: "A Starter Tutorial: Fitting sGP to a Synthetic Dataset"
author: "Ziang Zhang"
date: "2024-11-20"
...
For the purpose of illustration, we will take one of the synthetic datasets described in the main paper.
The dataset and its corresponding true function are shown below.
...
\text{log}(\lambda_i) &= g(x_i) + \xi_i, \\
g(x) &\sim \text{sGP}(\sigma_x), \\
\xi_i &\sim \text{N}(0, \sigma_\xi^2).
\end{aligned}
\end{equation}

We assume the sGP prior has a frequency of $2$ ($\alpha = 4\pi$), and we consider the following Exponential prior on the one-step PSD $\sigma_x(1)$ defined as:
\[
\text{P}(\sigma_x(1) > 2) = 0.1.
\]
