...
compute_mse_coverage <- function(mod, locations, func, level = 0.95){
  pred <- predict(mod, newdata = data.frame(x = locations), variable = "x", quantiles = c((1 - level)/2, level + (1 - level)/2))
  mse <- mean((func(locations) - pred$mean)^2)
...
sample_exact <- function(mod_exact, M = 3000){
  ...
  samps_exact_global <- as.matrix(sorted_data$cos_x, ncol = 1) %*% samps_exact$samps[mod_exact$fixed_samp_indexes$cos_x,,drop = F ] +
    as.matrix(sorted_data$sin_x, ncol = 1) %*% samps_exact$samps[mod_exact$fixed_samp_indexes$sin_x,,drop = F ] +
    as.matrix(rep(1, n), ncol = 1) %*% samps_exact$samps[mod_exact$fixed_samp_indexes$intercept,,drop = F ]
